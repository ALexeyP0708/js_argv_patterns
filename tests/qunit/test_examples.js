import {ArgvArray, ArgvElement, ArgvPattern} from "../../src/export.js";
        ...
        */
        ...
        if (help) {
            ...
            if (argv.get('start', {order: 1})) {
                ...
                check = true;
            } else if (argv.get('end', {order: 1})) {
                message.push(helpMessages['end']);
                check = true;
            }
            if (check === true && (argv.get('--force') || argv.searchElement({
                type: 'option',
                key: /[\w]+/i
            }).length <= 1 && diff.searchElement({type: 'option', key: /[\w]+/i}).length <= 0)) {
...
// other code  
        ...
        if (argv.get('--help')) {
            let message = [];
            let check = false;
            if (argv.get('start', {order: 1}) && (extArgv.get('--after') || argv.searchElement({
                    type: 'option',
                    key: /[\w]+/i
                }).length <= 1 &&
                diff.searchElement({type: 'option', key: /[\w]+/i}).length <= 0)) {
                message.push(extHelpMessages['--after']['start']);
            } else if (argv.get('end', {order: 1}) && (extArgv.get('--after') || argv.searchElement({
                    type: 'option',
                    key: /[\w]+/i
                }).length <= 1 &&
                diff.searchElement({type: 'option', key: /[\w]+/i}).length <= 0)) {
